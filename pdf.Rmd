...
Planteamos una especificación para buscar evidencia de que la discriminación tiene rendimientos decrecientes respecto a la experiencia (*yearsexp*):
...
```
Podemos observar que para el grupo base (blancos) los años de experiencia tienen un rendimiento decreciente en la probabilidad de que les hagan *call-back*, mientras que con las personas afroamericanas tiene rendimientos crecientes pero no significativos. Se ilustra en la siguiente gráfica:
...
d.34factor<-lm(call_back ~ (yearsexp + yearsexp2)*as.factor(black), data = nam)
mydf <- ggpredict(d.34factor, terms = c("black","yearsexp","years2"))
plot(mydf)
